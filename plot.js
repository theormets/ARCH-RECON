...
function makeBellPlot(mount, records, options={}) {
  ...
  function draw(){
    ...
    let content=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960 ${totalH}" class="plot-svg" role="img" aria-label="${esc(title)}: loop width versus height"><rect width="960" height="${totalH}" fill="#fffef9"/><text x="82" y="23" fill="#243a33" font-family="Arial,sans-serif" font-size="13">${esc(title)} · ${filtered?'filtered':'full dataset'} · ${shown.length} records</text><defs><clipPath id="${uid}"><rect x="${left}" y="${top}" width="${width}" height="${height}"/></clipPath></defs>`;
    for(let i=0;i<=5;i++){
      const x=left+i*width/5,y=top+i*height/5;
      content+=`<path d="M ${x} ${top} V ${top+height} M ${left} ${y} H ${left+width}" fill="none" stroke="#e4e7df" stroke-width="1"/><text x="${x}" y="470" text-anchor="middle" fill="#68736b" font-size="12" font-family="Arial">${Number((maxX*i/5).toFixed(2))}</text><text x="69" y="${y+4}" text-anchor="end" fill="#68736b" font-size="12" font-family="Arial">${Number((maxY*(5-i)/5).toFixed(2))}</text>`;
    }
    content+=`<path d="M 82 45 V 445 H 908" fill="none" stroke="#52665a"/><text x="495" y="500" text-anchor="middle" font-family="Arial" font-size="14" fill="#243a33">Loop outer width / diameter (mm)</text><text transform="translate(22 245) rotate(-90)" text-anchor="middle" font-family="Arial" font-size="14" fill="#243a33">Loop height h (mm)</text><g clip-path="url(#${uid})">`;
    ...
    if(hook){const pts=[];for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?3.3:8;pts.push(`${X(6.45)+r*Math.cos(a)},${Y(8.16)+r*Math.sin(a)}`);}content+=`<polygon class="reference-star" points="${pts.join(' ')}" fill="#ba332b" stroke="#ffffff" stroke-width="1" tabindex="0" role="button" aria-label="Ambal reference: width 6.45, whole component height 8.16"><title>Ambal reference: 6.45 × 8.16; whole component height, not ring-only</title></polygon>`;}
    content+='</g>';
    content+=`<text x="82" y="524" font-family="Arial" font-size="11" fill="#7b5140">${hook?'★ Ambal reference: 6.45 × 8.16 — height includes the surviving base.':'Dataset only — Ambal reference is hidden.'}</text>`;
    legendLines.forEach((l,i)=>{const y=549+i*20;content+=(l.i===0?`<circle cx="87" cy="${y-4}" r="4" fill="${color(l.g)}"/>`:'')+`<text x="99" y="${y}" font-family="Arial" font-size="11" fill="#52665a">${esc(l.text)}</text>`;});
  ...
  mount.querySelectorAll('[data-format]').forEach(b=>b.onclick=async()=>{
    try{const format=b.dataset.format,base=`BellRecon_${filtered?'Filtered':'Full'}_${field}_${hook?'With_Reference':'Dataset_Only'}`;
      if(format==='HTML'){
        ...
        const text='<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+esc(title)+' — BellRecon</title><style>body{max-width:1100px;margin:30px auto;padding:15px;font:14px/1.6 Arial;background:#f6f3ec;color:#243a33}.plot-svg{width:100%;height:auto}.plot-controls,.export-row{display:flex;gap:14px;flex-wrap:wrap;margin:15px 0}button,select{padding:7px;background:#fffef9;border:1px solid #c7cec2;color:inherit}.plot-selection{padding:14px;background:#e5e9de}.micro{font-size:12px}.plot-count{font-size:12px}h1,h3{font-family:Georgia}</style><h1>BellRecon</h1><p>Source-linked comparative bell dimensions. Image-scaled estimates, not precision metrology.</p><main id="plot"></main><script>('+makeBellPlot.toString()+')(document.getElementById("plot"),'+jsData+','+jsOptions+');<\/script></html>';
